...
const AdminFlightTickets = () => {
  ...
  const uploadMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("bookings").update({ ticket_status: newStatus, flight_provider: flightProvider || selectedBooking.flight_provider, ticket_file_url: fileUrl, admin_ticket_message: adminMessage || selectedBooking.admin_ticket_message } as any).eq("id", selectedBooking.id);
      if (error) throw error;
    },
    onSuccess: () => {
      toast({ title: "Flight ticket updated" }); queryClient.invalidateQueries({ queryKey: ["admin-ticket-bookings"] });
      setSelectedBooking(null); setUploadFile(null); setFlightProvider(""); setAdminMessage("");
    },
  ...
  const openUploadDialog = (booking: any) => {
    setSelectedBooking(booking); setFlightProvider(booking.flight_provider || ""); setAdminMessage(booking.admin_ticket_message || "");
    setNewStatus(booking.ticket_status === "pending" ? "uploaded" : booking.ticket_status); setUploadFile(null);
  };
    ...
    <div className="space-y-6">
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={9} className="text-center py-8 text-muted-foreground">
                    <Plane className="h-8 w-8 mx-auto mb-2 opacity-30" />No pilgrims found
                  </TableCell>
                </TableRow>
              ) : (
                paginatedItems.map((b: any, index) => {
                  ...
                  return (
                    <TableRow key={b.id}>
                      ...
                      <TableCell><Badge className={ticketStatusColors[b.ticket_status] || "bg-muted"}>{b.ticket_status}</Badge></TableCell>
                      <TableCell>
                        <div className="flex items-center justify-center gap-2">
                          {b.ticket_file_url && (
                            <>
                              <button className="h-8 w-8 rounded-lg bg-muted/80 text-muted-foreground flex items-center justify-center hover:bg-muted transition-colors" onClick={() => handleViewFile(b.ticket_file_url)} title="View">
                                <Eye className="h-4 w-4" />
                              </button>
                              <button className="h-8 w-8 rounded-lg bg-primary/15 text-primary flex items-center justify-center hover:bg-primary/20 transition-colors" onClick={() => handlePrintFile(b.ticket_file_url)} title="Print">
      ...
      <Dialog open={!!selectedBooking} onOpenChange={(open) => !open && setSelectedBooking(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Manage Flight Ticket — {selectedBooking?.full_name}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label>Ticket Status</Label><Select value={newStatus} onValueChange={setNewStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="pending">Pending</SelectItem><SelectItem value="uploaded">Uploaded</SelectItem><SelectItem value="approved">Approved</SelectItem><SelectItem value="rejected">Rejected</SelectItem></SelectContent></Select></div>
            <div><Label>Airlines / Flight Provider</Label><Input placeholder="e.g. Max Air, Flynas..." value={flightProvider} onChange={(e) => setFlightProvider(e.target.value)} /></div>
            <div><Label>Upload Ticket Document (PDF/Image)</Label><Input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setUploadFile(e.target.files?.[0] || null)} />{selectedBooking?.ticket_file_url && !uploadFile && <p className="text-xs text-muted-foreground mt-1">Current file: {selectedBooking.ticket_file_url.split("/").pop()}</p>}</div>
            ...
            <Button className="w-full" onClick={() => uploadMutation.mutate()} disabled={uploadMutation.isPending}>{uploadMutation.isPending ? "Saving..." : "Save Ticket Details"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
